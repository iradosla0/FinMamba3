...
def _autocorr_1d(x: np.ndarray, lag: int) -> float:
    """Sample autocorrelation of a 1-D array at the given lag.

    Returns nan when the array is too short or has near-zero variance so
    callers can safely log the result without crashing on degenerate rollouts.
    """
    if x.size < lag + 2:
        return float("nan")
    x = x - x.mean()
...
def _imagine_and_log(
    ...
    # Stylized-fact fidelity: autocorrelation structure of imagined trajectories.
    # Volatility clustering: |Δmid| autocorrelation should be positive at short lags.
    # A collapsed or random-walk rollout will show near-zero autocorrelation.
    dmid = np.diff(mid_norm)
    abs_dmid = np.abs(dmid)
    wlogger.log("Imagine/absret_autocorr_lag1",  _autocorr_1d(abs_dmid, 1),  global_step=global_step)
    wlogger.log("Imagine/absret_autocorr_lag5",  _autocorr_1d(abs_dmid, 5),  global_step=global_step)
    wlogger.log("Imagine/absret_autocorr_lag10", _autocorr_1d(abs_dmid, 10), global_step=global_step)
    # Microstructure mean-reversion: sign autocorrelation should be weakly negative.
    wlogger.log("Imagine/sign_autocorr_lag1", _autocorr_1d(np.sign(dmid + 1e-12), 1), global_step=global_step)
    # Spread persistence: spreads are strongly persistent in real LOBs (autocorr ≈ 0.9+).
    wlogger.log("Imagine/spread_autocorr_lag1", _autocorr_1d(spread_norm, 1), global_step=global_step)
...
def _validation_metrics(
    ...
    with torch.no_grad(), torch.autocast(
        ...
        prior_logits = world_model.dist_head.forward_prior(
            world_model.condition_dist_feat(dist_feat)[:, :-1]
        )
    ...
    # Per-step MSE (B, L-1) — used by calibration and regime-gap below.
    per_step_mse_np = (diff ** 2).mean(dim=-1).cpu().numpy()
    # Identity baseline: the trivial predictor that forecasts obs[t+1] = obs[t].
    # Any useful dynamics model must beat this on average; the ratio < 1 means the
    # prior is better than copying the last observation.
    identity_diff = obs[:, 1:].detach().float() - obs[:, :-1].detach().float()
    identity_feature_mse = (identity_diff ** 2).mean(dim=(0, 1)).cpu().numpy()
    identity_next_mse = float(identity_feature_mse.mean())
    prior_next_mse = float(feature_mse.mean())
    prior_vs_identity_ratio = prior_next_mse / max(identity_next_mse, 1e-10)
    ...
    # -- Calibration: does prior entropy predict prior error? --
    # A well-calibrated prior should be more uncertain before volatile ticks.
    # Positive correlation here means the prior "knows when it doesn't know".
    with torch.no_grad():
        log_p = torch.log_softmax(prior_logits.float(), dim=-1)   # (B, L-1, C, D)
        prior_entropy_per_step = (
            -(log_p.exp() * log_p).sum(dim=-1).mean(dim=-1).cpu().numpy()  # (B, L-1)
        )
    ent_flat = prior_entropy_per_step.reshape(-1)
    mse_flat = per_step_mse_np.reshape(-1)
    finite_cal = np.isfinite(ent_flat) & np.isfinite(mse_flat)
    if finite_cal.sum() > 10:
        prior_entropy_mse_corr = float(
            np.corrcoef(ent_flat[finite_cal], mse_flat[finite_cal])[0, 1]
        )
    else:
        prior_entropy_mse_corr = float("nan")

    # -- Window-level volatility regime gap --
    # Split the sampled windows at the median realized vol and compare next-step MSE
    # and direction accuracy between low-vol and high-vol halves. A model that
    # generalises across regimes should show a small gap; one that has overfit to
    # quiet periods will show a large gap.
    mid_idx_vol = world_model.midprice_index
    obs_np_for_vol = obs.detach().float().cpu().numpy()  # (B, L, F)
    dmid_vol = np.diff(obs_np_for_vol[:, :, mid_idx_vol], axis=1)  # (B, L-1)
    window_vol = dmid_vol.std(axis=1)  # (B,)
    vol_median = float(np.median(window_vol))
    low_vol_mask = window_vol <= vol_median
    high_vol_mask = ~low_vol_mask
    if low_vol_mask.sum() > 0 and high_vol_mask.sum() > 0:
        per_window_mse = per_step_mse_np.mean(axis=1)  # (B,)
        regime_low_mse = float(per_window_mse[low_vol_mask].mean())
        regime_high_mse = float(per_window_mse[high_vol_mask].mean())
        regime_gap_mse = regime_high_mse - regime_low_mse
        # Per-window direction accuracy (avoid the flattened nonzero mask from above).
        per_window_dir = np.full(obs_np_for_vol.shape[0], float("nan"))
        for bi in range(obs_np_for_vol.shape[0]):
            td_i = true_delta[bi]  # (L-1,)
            pd_i = pred_delta[bi]  # (L-1,)
            nz_i = np.abs(td_i) > 1e-7
            if nz_i.any():
                per_window_dir[bi] = float(
                    (np.sign(td_i[nz_i]) == np.sign(pd_i[nz_i])).mean()
                )
        regime_low_dir = float(np.nanmean(per_window_dir[low_vol_mask]))
        regime_high_dir = float(np.nanmean(per_window_dir[high_vol_mask]))
        regime_gap_dir = regime_high_dir - regime_low_dir
    else:
        regime_low_mse = regime_high_mse = regime_gap_mse = float("nan")
        regime_low_dir = regime_high_dir = regime_gap_dir = float("nan")

    # -- Direction head accuracy at the configured bucket threshold --
    # This is the discriminative head's accuracy, distinct from the decoded-prior
    # direction accuracy already reported as Val/mid_direction_accuracy.
    dir_head_accuracy = float("nan")
    if world_model.use_direction_head and world_model.direction_head is not None:
        thr = world_model.direction_threshold
        with torch.no_grad():
            cond_for_dir = world_model.condition_dist_feat(dist_feat[:, :-1])
            dir_logits = world_model.direction_head(cond_for_dir.float())
            dir_preds = dir_logits.argmax(dim=-1).cpu().numpy()  # (B, L-1)
        dmid_norm = np.diff(obs_np_for_vol[:, :, mid_idx_vol], axis=1)  # (B, L-1)
        dir_targets = np.where(dmid_norm > thr, 2, np.where(dmid_norm < -thr, 0, 1))
        dir_head_accuracy = float((dir_preds == dir_targets).mean())
    ...
    metrics = {
        ...
        # Identity baseline and prior/identity comparison.
        "Val/identity_baseline_mse": identity_next_mse,
        "Val/prior_vs_identity_ratio": prior_vs_identity_ratio,
        # Calibration: positive = prior is more uncertain before larger errors.
        "Val/prior_entropy_mse_corr": prior_entropy_mse_corr,
        # Regime gap: high_vol minus low_vol on each metric.
        "Val/regime_gap/normalized_next_mse": regime_gap_mse,
        "Val/regime_gap/mid_direction_accuracy": regime_gap_dir,
        "Val/regime_gap/low_vol_mse": regime_low_mse,
        "Val/regime_gap/high_vol_mse": regime_high_mse,
        "Val/regime_gap/low_vol_dir_acc": regime_low_dir,
        "Val/regime_gap/high_vol_dir_acc": regime_high_dir,
        # Direction head accuracy at the configured bucket threshold.
        "Val/direction_head_accuracy": dir_head_accuracy,
    }
